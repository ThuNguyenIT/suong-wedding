        ...
        {scrollSnaps.map((_, index) => (
          <motion.button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              index === selectedIndex ? 'bg-white scale-125 shadow-lg' : 'bg-white/50 hover:bg-white/70'
            }`}
            whileHover={{ scale: 1.2 }}
            whileTap={{ scale: 0.9 }}
            animate={{
              scale: index === selectedIndex ? 1.25 : 1,
              backgroundColor: index === selectedIndex ? 'rgba(255, 255, 255, 1)' : 'rgba(255, 255, 255, 0.5)',
            }}
            transition={{ duration: 0.2 }}
          />
        ))}
